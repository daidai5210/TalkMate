...
const variantClass: Record<TMButtonVariant, string> = {
  primary:
    'bg-brand-600 text-white shadow-brand hover:bg-brand-700 disabled:bg-slate-300 disabled:shadow-none',
  secondary:
    'border border-app-border bg-app-surface text-slate-700 shadow-sm hover:bg-app-muted disabled:bg-slate-100 disabled:text-slate-400',
  ...
  ghost:
    'bg-white/15 text-white hover:bg-white/25 disabled:bg-white/10 disabled:text-white/50 backdrop-blur-sm',
};

const sizeClass: Record<TMButtonSize, string> = {
  sm: 'min-h-10 rounded-app px-3 text-xs',
  md: 'min-h-touch rounded-app px-4 text-sm',
  lg: 'min-h-12 rounded-app px-5 text-base',
};
