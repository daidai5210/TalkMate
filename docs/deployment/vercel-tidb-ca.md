...
1. 后端启动创建 SQLAlchemy engine 时读取 `TIDB_CA_PEM`。
2. 当 `DATABASE_URL` 使用 `mysql` / `mysql+pymysql` 且 `TIDB_CA_PEM` 存在时，后端将 PEM 内容写入 `/tmp/tidb-ca.pem`。
3. SQLAlchemy 通过 PyMySQL `connect_args={"ssl": {"ca": "/tmp/tidb-ca.pem"}}` 使用该运行时 CA 文件。
4. 当 `TIDB_CA_PEM` 不存在时，MySQL/TiDB 连接仅保留 `pool_pre_ping=True`，不注入本地 CA 路径。
5. SQLite 本地开发与测试仍使用 `check_same_thread=False`，不受 `TIDB_CA_PEM` 影响。
